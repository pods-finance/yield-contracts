...
import { STETH, ConfigurationManager, InvestorActorMock, PrincipalProtectedMock, WrapperAsYieldSourceMock, WstETH } from '../../typechain'

describe('PrincipalProtectedMock Wrapper', () => {
  let asset: WstETH, vault: PrincipalProtectedMock, yieldSource: WrapperAsYieldSourceMock,
    investor: InvestorActorMock, configuration: ConfigurationManager, stEthContract: STETH
  ...
  before(async () => {
    ...
    // Wrapper of wstETH
    const assetFactory = await ethers.getContractFactory('WstETH')
    const WrapperAsYieldSourceMockFactory = await ethers.getContractFactory('WrapperAsYieldSourceMock')

    stEthContract = await stEthFactory.deploy()
    asset = await assetFactory.deploy(stEthContract.address)
    yieldSource = await WrapperAsYieldSourceMockFactory.deploy(asset.address)
  ...
  it('Should remove the same balances even if no processed queue happened during a round interval', async () => {
    // This test will only work if InvestRatio = 50%
